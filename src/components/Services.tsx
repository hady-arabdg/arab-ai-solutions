...
const Services = () => {
  const { t } = useLanguage();
  const sectors = [
    { icon: Building2, title: t("services.realestate.title"), description: t("services.realestate.desc"), image: realEstateImage, link: "/real-estate" },
    { icon: Briefcase, title: t("services.services.title"), description: t("services.services.desc"), image: servicesImage, link: "/services" },
  ];
      ...
      <div className="container mx-auto relative z-10">
        <div className="text-center mb-16 animate-fade-in">
          <h2 className="text-4xl md:text-5xl font-bold mb-4">
            <span className="bg-gradient-to-r from-primary to-secondary bg-clip-text text-transparent">{t("services.title")}</span>
          </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl mx-auto">
          {sectors.map((sector, index) => (
            <Link key={index} to={sector.link}>
              <Card className="group bg-card/50 backdrop-blur-sm border-primary/20 hover:border-primary/50 transition-all duration-500 hover:shadow-[var(--shadow-card)] hover:scale-105 overflow-hidden animate-slide-up cursor-pointer" style={{ animationDelay: `${index * 0.2}s` }}>
                {sector.image && (
        ...
        <div className="mt-20 text-center bg-gradient-to-br from-card to-card/50 backdrop-blur-sm rounded-2xl p-12 border border-primary/20 animate-fade-in">
          ...
          <p className="text-xl text-muted-foreground mb-8 max-w-2xl mx-auto">{t("services.cta.subtitle")}</p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <a href="https://wa.me/966549793708" target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center gap-2 px-8 py-4 bg-[#25D366] hover:bg-[#20BA5A] text-white font-semibold rounded-xl transition-all hover:scale-105 shadow-lg">
