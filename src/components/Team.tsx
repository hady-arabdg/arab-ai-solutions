...
const Team = () => {
  const { t } = useLanguage();
  const cards = [
    { icon: MapPin, color: "primary", title: t("team.card1.title"), desc: t("team.card1.desc") },
    { icon: Users, color: "secondary", title: t("team.card2.title"), desc: t("team.card2.desc") },
    { icon: Award, color: "accent", title: t("team.card3.title"), desc: t("team.card3.desc") },
    { icon: Headset, color: "primary", title: t("team.card4.title"), desc: t("team.card4.desc") },
  ];
  return (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
          {cards.map((c, i) => (
            <div key={i} className="bg-card/30 backdrop-blur-sm rounded-2xl p-8 border border-primary/20 hover:border-primary/50 transition-all hover:scale-105 hover:shadow-[var(--shadow-card)] animate-slide-up" style={{ animationDelay: `${i * 0.1}s` }}>
              <div className={`mb-4 inline-block p-4 rounded-xl bg-${c.color}/10`}>
                <c.icon className={`w-8 h-8 text-${c.color}`} />
              </div>
